...
/*
    ...
    GaussianBlur(input_gray_image, input_gray_image, cv::Point(5, 5), 0);
    //이미지를 이진화 (입력이미지,출력이미지,...)
    adaptiveThreshold(input_gray_image, roi[0], 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 17, 3);
    //이미지 잡음 제거 (입력이미지,출력이미지...)
    morphologyEx(roi[0], roi[0], cv::MORPH_CLOSE, kernel); //close
    //색 반전: deliate연산을 위해
    bitwise_not(roi[0], roi[0]);
    //팽창 연산, 이미지의 하얀부분을 팽창시킨다 (입력이미지, 출력이미지,...,반복횟수)
    dilate(roi[0], roi[0], mask, cv::Point(-1, -1), 3);
*/
    int cnt = 0; //이미지 저장 이름 구별을 위한 변수
